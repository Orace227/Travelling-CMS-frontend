...
export default function GetConfirmedInquiry() {
  ...
  const handleConfirm = async (row) => {
    try {
      ...
      const UpdatedInquiry = await axios.post('/UpdateInquiry', { _id: row._id, isConfirm: true });
      console.log('get data', UpdatedInquiry.data);
      window.location.reload();
    } catch (error) {
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    // console.log(row);
                    const { _id, packageId, isConfirm, fullName, mobile, email, budget } = row;
